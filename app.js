(() => {
  ...
  function loadSettings(){
    ...
    if(!el.patInput.value) el.settingsPanel.classList.add('open');
  }
  ...
  async function poll(){
    ...
    try{
      ...
      else if(run.status==='completed'){stopPoll();stopTimer();
  ...
  async function stop(){
    if(!runId)return;
    try{await api(`/repos/${repo()}/actions/runs/${runId}/cancel`,{method:'POST'});toast('Cancelled','info');stopPoll();stopTimer();
  ...
  function updateSteps(cur){
    ...
    $$('.step').forEach(s=>{const i=order.indexOf(s.dataset.step);
